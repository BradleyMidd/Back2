...
//Maak variables aan om de info van lid te pakken via lid nieuw pagina
   ...
   strlen($member_since) > 0)
{
	...
	{
        //Maak de query aan dat je een lid kan toevoegen
		$query = "INSERT INTO back2_leden (id, birth_date, first_name, last_name, gender, member_since) VALUES (NULL, '$birth', '$first_name', '$last_name', '$gender', '$member_since')";

		//Voer de query uit
		$result = mysqli_query($mysqli, $query);

        //Als de query goed uitgevoerd is, dan is het toevoegen gelukt
		if($result)
		{
            echo "<script>
        ...
        //Anders geeft hij een melding dat het toevoegen mis ging
		else
		{
            echo "<script>
			alert('Er ging wat mis met het toevoegen van lid');
		  </script>";
            echo "<script>
			  window.history.back();
		  </script>";
...
else
{
    ...
    if(strlen($birth) < strlen($member_since))
    {
        echo "<script>
